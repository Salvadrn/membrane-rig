...
class Config:
    ...
    def validate(self) -> None:
        ...
        if self.valve.type not in ("servo", "servo_kpwm", "pwm", "none"):
            errs.append(f"valve.type must be servo, servo_kpwm, pwm or none, "
                        f"got {self.valve.type!r}")
        # "servo_kpwm" = same servo, pulses generated by the KERNEL's hardware PWM
        # instead of pigpio. Added 2026-08-07 because on this Pi (Trixie, pigpio
        # built from source) the servo misbehaves on every pigpio-generated pulse
        # — measured uncoupled and unloaded, with both pigpio clock peripherals,
        # so nothing mechanical explains it. Kept as a separate type rather than a
        # flag on "servo": which generator produced a run's pulses is something
        # the run metadata has to be able to state.
