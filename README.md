...
```
GPIO18 ──[470Ω]── gate │ logic-level N-MOSFET (IRLZ44N)
   valve coil: +12V ── coil ── MOSFET drain ;  source ── GND
   flyback diode (1N5819/SB560) across coil, cathode → +12V
```
PWM duty sets coil current → valve position. This topology is not built (the rig
uses `type: servo`), and one caveat matters if it ever is: a bare 3.3 V GPIO
cannot drive this gate at 1 kHz with fine duty resolution at *any* series
resistance — the Miller plateau (~2.5 V) leaves only ~0.8 V of overdrive, so the
...
- Every threaded port needs an **NPT×barb** adapter to reach the silicone, every
  barb a **clamp**, and the sensor a **tee**. The 12V barrel
...
- Add a **10k gate-to-source pulldown** (so the diverter can't energise at boot
  before the code runs) + a **470 Ω** series gate resistor on the MOSFET — the
  BCM2711 GPIO sources ~8 mA and 220 Ω would draw ~15; the slow edge is harmless
  on a diverter that switches only a few times per run.
