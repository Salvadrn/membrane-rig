...
**Gamma deck:** _(URL filled in after generation — see docs/talk/README.md)_
...
> _FIGURE: fig4_sequence.png_
"Now the results — and these are all from **simulation**, running the exact production code against a modeled plant. Against a ±8 kPa oscillating supply — worse than the real bench should be — the loop held each setpoint to **two-tenths to seven-tenths of a kPa**. Across a 15 °C sweep the derived k held constant to **five significant figures** while the raw slope moved **39%** — the cancellation I promised, proven end to end. And it reproduced a real, hand-taken dataset: k of 1.44e-12, pore size 6.8 microns, R-squared 0.994."
...
1. Slide 6 — one sentence: "k comes out independent of temperature, which is also how the rig proves it's working."
2. Slide 2 — drop the "factor of ten" clause; keep pore-size and repeatable.
**Never cut slide 4.** It's the idea a mentor will remember.

## If you're short
- Slide 5 — walk the actual fit on `fig5`: point at one dot, "each dot is one 60-second collection."
- Slide 8 — name the plot on `fig4`: "the wobble shows up in the valve, so it doesn't show up in the pressure."
