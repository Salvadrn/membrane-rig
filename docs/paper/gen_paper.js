...
children.push(P("The apparatus is not one vessel but a short chain of pieces. Compressed air from the laboratory gas panel is admitted through a quarter-turn ball valve into the headspace of a stainless-steel water tank; a dip tube reaching the tank floor carries the pressurized distilled water up and out, along a line carrying the existing analog pressure gauge, and into a separate cylindrical holder in which the membrane specimen — about a centimetre square — is clamped. The automation adds four elements to that chain (Fig. 1): (i) a hobby servomotor (20 kg·cm class) coupled through a 3-D-printed 2:1 reduction to the quarter-turn ball valve on the air inlet — the element that sets the pressure for everything downstream; (ii) a ratiometric pressure transducer (0–103.4 kPa, 0.5–4.5 V) teed into the line beside the existing analog gauge, deliberately at the holder inlet rather than back at the tank: the dip tube and line drop pressure in proportion to the flow they carry, so measuring at the holder leaves that drop upstream of the reading instead of between the sensor and the specimen; (iii) a waterproof DS18B20 digital thermometer immersed in the permeate stream, so viscosity is evaluated at the temperature of the water that crossed the membrane; and (iv) a 12 V three-way solenoid valve on the holder outlet that routes the permeate either to waste or to the graduated measurement container. An adjustable mechanical pressure-relief valve on the air side is on order as a hardware overpressure backstop but is not yet fitted; Section 8.1 states plainly what that costs today."));
children.push(figure("fig1_system.png", 564, 338));
children.push(caption("Figure 1. System block diagram. The controller closes two loops: pressure (transducer → PID → servo-actuated air valve) and collection timing (sequencer → solenoid diverter). Every protective layer shown is inside the control system: no mechanical relief valve is fitted (Section 8.1)."));
...
children.push(EQ([RI("t"), SUB("pulse"), RI(" = 1130 − 6.10 · u   µs")], 7));
children.push(P("i.e., 1130 µs at 0% and 520 µs at 100%, both measured at the valve. The sense is inverted deliberately: with the direct mapping, bench measurement showed the 0% command — the state the rig falls back to on every abort, at start-up, and on a sensor fault — driving the valve toward open. The endpoints are calibrated to the valve's useful travel rather than the servo's mechanical limits, and that is not a generic margin: the shut endpoint sits deliberately short of the handle stop, because the servo now holds its shut position rather than being released, and one held against a hard stop stalls and hunts audibly — observed on the bench when the endpoint was first set on the stop itself. The pulses are generated by the pigpio daemon, which times them by DMA in hardware; software-timed PWM on a multitasking operating system jitters by tens of microseconds, which across the calibrated 610 µs span is several percent of the valve’s entire useful travel — visible chatter. Two properties of this chain matter for control. First, a ball valve's flow characteristic is strongly nonlinear over its quarter-turn travel — 82° of useful travel on this valve, the last few degrees being the handle stop rather than sealing, but the operating point here sits near the closed end — the flattest region — and the loop gain was tuned there. Second, and central to the design philosophy: the regulated variable of record is always the independently measured transducer pressure, never the valve position. The valve is merely how the controller pushes; the transducer is what the physics is asked about. One qualification, developed in Section 8.3: 0% is the bottom of the regulating range, not necessarily a fully sealed valve, which is why the end of a run invokes a distinct full-close rather than simply releasing the loop — though the separate seating pulse that action is meant to use has not been calibrated, so today it lands on the 0% endpoint itself."));
...
children.push(figure("fig2_software.png", 564, 327));
...
children.push(figure("fig3_control.png", 583, 282));
...
children.push(figure("fig4_sequence.png", 583, 328));
...
children.push(P([
  R("When the volumes are entered, the system fits Eq. (2) to the points ("), RI("P̄"), SUB("i"), RI(", Q"), SUB("i"), R(") — pooled across the completed items of the playlist (Section 5.2), excluding any run flagged as having continued under a raised ceiling (Section 8.1), where "), RI("P̄"), SUB("i"), R(" is the "), RI("measured mean"), R(" pressure of collection window "), RI("i"), R(", not the nominal setpoint — by ordinary least squares:"),
]));
children.push(EQ([
  RI("b = Σ(P̄"), SUB("i"), RI(" − P̄)(Q"), SUB("i"), RI(" − Q̄) / Σ(P̄"), SUB("i"), RI(" − P̄)²"),
], 14));
children.push(P([
  R("then converts the slope to SI units ("), RI("b"), SUB("Pa"), RI(" = b"), R("/1000 for pressures logged in kPa) and applies Eqs. (3)–(4) with "), RI("μ"), R(" evaluated at the run-mean measured water temperature. Regressing against measured means is the analytical half of the design argument begun in Section 1.2: the regulation layer's job is only to keep each cluster of pressures near its target; the analysis never pretends the pressure was anything other than what the transducer recorded. Coarse regulation therefore widens the scatter of points along the fitted line — which R² reports honestly — but does not displace the line. The fit, the derived k and "), RI("d"), SUB("h"), R(", and the R² verdict are computed automatically the moment volumes are entered. R² says the points lie on a line; the standard error says how firmly its slope is fixed — with three points the first can hide the second. Every run leaves a complete, self-describing evidence trail on disk:"),
]));
children.push(...bullets([
  "A 20 Hz time-series CSV — timestamp, phase, setpoint, measured pressure, valve command, diverter state, in-band flag, and the water temperature alongside how it was obtained — sufficient to reconstruct the entire run. That last pair is written per sample rather than once per run, because a probe that stops answering keeps its last value while no longer measuring, and one label for the whole run would misdescribe every row on one side of that moment. The run-level summaries carry the same field, and when it did change partway they report it as mixed rather than asserting a single origin the run did not have.",
  ...
  "An analysis JSON with the fit (slope, intercept, R²), k and the standard error of the slope propagated onto it, pore size, viscosity, and the temperature with its provenance — viscosity follows from that temperature and k from the viscosity, so the permeability carries the origin of its own inputs instead of leaving it to be reconstructed from a configuration file that may since have changed.",
...
children.push(figure("fig5_fit.png", 508, 334));
children.push(caption("Figure 5. The chart the instrument itself produces for the simulated run of Fig. 4: flow rate versus measured mean pressure, the fitted line (Eq. 2), and the derived permeability with its standard error and pore size, annotated with the water temperature and viscosity used."));
...
children.push(P("Protection is layered so that no single failure — software, sensor, or actuator — can leave the vessel pressurizing uncontrolled (Table 3). The safe state is “air inlet sealed, diverter to waste”: with the feed shut, the vessel self-depressurizes through the membrane, and the diverter reaches waste by de-energizing, so it fails safe on any power loss. The servo does not share that property, and bench measurement made it worse than assumed: released, it does not hold the commanded angle but drifts off it. On power loss the air valve therefore neither seals nor stays where it was, and which way it drifts, and how far, has not been characterized — an unmeasured quantity that decides whether a power cut leaves the feed shut or part open. The mechanical relief valve that would cover exactly that case is on order but not yet fitted, and a purchase order is not a protective layer. The consequence deserves stating without euphemism: every layer in Table 3 requires the controller to be alive. Should the single-board computer hang or lose power with the air valve open, nothing in the instrument closes it, and the pressure the vessel reaches is bounded only by what the gas panel can deliver — a bound nobody has measured, which is worse than a high one. Two operating measures follow, neither of them a substitute: the supply regulator must be set low enough that its maximum delivery is itself a pressure the specimen tolerates, and the software keeps the operator ceiling-raise of Section 8.1 disabled until that number exists; and the panel valve is closed by hand at the end of a session rather than trusting the servo to hold. Fitting the relief valve remains the highest-value hardware addition to this instrument (Section 10)."));
...
children.push(table([3120, 3120, 3120], [
  ...
  ["Per-run ceiling", "min(max(setpoint) + 10, specimen limit)", "Hold at safe state, alarm"],
...
children.push(...bullets([
  ...
  "No hardware-only protective layer yet. The mechanical relief valve is on order but not fitted, and until it is mounted and set the limitation stands: every layer of Table 3 depends on the controller running (Section 8). Two passive measures restore part of it, and the order matters: lowering the air regulator's setting is free and bounds the worst case at its source — a dead controller with the valve open cannot take the cell past what the panel delivers — so it comes first; fitting the relief valve then covers what a regulator cannot, its own failure open, and is the only measure that vents rather than merely bounds.",
  "Actuator torque margin. The valve-stem breakaway torque has not been measured; the printed transmission carries a 2:1 reduction as margin, with a higher-torque servo as fallback.",
  "Manual volume reading. Permeate volume is still read by eye — the last operator-dependent step. It is benign in a useful way: a fixed ±δV over a common window is the same absolute error in every flow rate, so it widens the fitted slope's uncertainty without biasing it, and preserves the equal-variance assumption behind the unweighted fit of Section 6 — about 0.4% in k at ±5 mL over 60 s, a window itself configurable from the rig's real flow. An electronic scale on the volume-entry API would close the step and give continuous flow curves.",
  "Feed-forward. If compressor cycling exceeds what feedback alone rejects, a second transducer on the supply side (three ADC channels are spare) would let the controller see a dip before the vessel does.",
]));
...
// References
children.push(H1("References", { keepNext: true }));
const refs = [
