...
children.push(EQ([RI("t"), SUB("pulse"), RI(" = 1167 − 6.47 · u   µs")], 7));
children.push(P("i.e., 1167 µs at 0% and 520 µs at 100%, both measured at the valve. The sense is inverted deliberately: with the direct mapping, bench measurement showed the 0% command — the state the rig falls back to on every abort, at start-up, and on a sensor fault — driving the valve toward open. The endpoints are calibrated to the valve's useful travel rather than the servo's mechanical limits (over-driving a stalled servo overheats it). The pulses are generated by the pigpio daemon, which times them by DMA in hardware; software-timed PWM on a multitasking operating system jitters by tens of microseconds, which across the calibrated 647 µs span is several percent of the valve’s entire useful travel — visible chatter. Two properties of this chain matter for control. First, a ball valve's flow characteristic is strongly nonlinear over its quarter-turn travel — 87° on this valve as measured, but the operating point here sits near the closed end — the flattest region — and the loop gain was tuned there. Second, and central to the design philosophy: the regulated variable of record is always the independently measured transducer pressure, never the valve position. The valve is merely how the controller pushes; the transducer is what the physics is asked about. One qualification, developed in Section 8.3: 0% is the bottom of the regulating range, not necessarily a fully sealed valve, so the end of a run drives the servo to a separate, calibrated seating position rather than leaving it at 0%."));
